...
function Login() {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      // Check if result and result.data exist before accessing
      ...
      >
        ...
        <div className="relative w-full">
          <input
            ...
            placeholder="Password"
            required
            className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 transition pr-16"
            value={password}
            onChange={(e) => setpassword(e.target.value)}
          />
          <span
            className="absolute cursor-pointer right-3 top-1/2 transform -translate-y-1/2 text-[#10c1f7] font-semibold text-sm"
            onClick={() => setShow((prev) => !prev)}
          >
            {show ? "hide" : "show"}
          </span>
